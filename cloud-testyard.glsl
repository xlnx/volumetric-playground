...
#define RAYMARCH_ITER_STEP .04
#define RAYMARCH_MAX_ITER 3
...
void main()
{
	...
	for (int i = 0; i < RAYMARCH_MAX_ITER; ++i)
	{
		if (z.a > 0.99) break;
		vec3 pos = ro + t * rd;
		float den = fbm(pos + vec3(0, 0, iGlobalTime));
		vec4 col = vec4(mix(vec3(1.), vec3(0.), den), den);
		col.rgb *= col.a;
		z += col * (1.0 - z.a);
		t += max(RAYMARCH_ITER_STEP, (1. - den) * t);
	}
	// z = vec4(fbm(ro * 10.) * .5 + .5);

	gl_FragColor = z;// * (1. - d);
	// clamp(col, 0., 1.);
	// gl_FragColor = vec4(fbm(vec3(tex * 10., iGlobalTime)));
}